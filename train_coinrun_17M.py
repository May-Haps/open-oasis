"""
Scaling ablation — ~17.0M parameter CoinRun world model (M variant).
1 epoch only; same hyperparameters as train_coinrun.py for fair comparison.

Model: DiT hidden=288, depth=6, heads=8  →  ~17.9M params
       (geometric series point between 9.2M and 31.4M)

FLOPs (6ND formula, S=2048 tokens, B=256 effective):
    Per sample : 6 × 17.9M × 2048 = 220 GFLOPs
    Per step   : 220 × 256         =  56 TFLOPs
    1 epoch    :  56 × 40,700      = 2.3 PFLOPs

Wall time on 2× H100 SXM:
    Speed   : ~8 it/s  (17.9/57.8 × 3.3 it/s baseline, with overhead)
    1 epoch : 40,700 / 8 ≈ 5,100 s ≈ 1.4 hours

Run:
    torchrun --nproc_per_node=2 train_coinrun_17M.py

Resume:
    torchrun --nproc_per_node=2 train_coinrun_17M.py --resume runs/coinrun_17M/ckpt_step_XXXXXXX.pt
"""
...
CONFIG = {
    ...
    # Training — 6 epochs for equal-FLOPs vs 57.8M baseline at 73K steps (13.3 ExaFLOPs)
    "epochs":          6,
    "batch_size":      128,
    "lr":              1e-4,
    "weight_decay":    0.01,
